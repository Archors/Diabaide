<template>
  <div>
    ...
    <div v-if="!Show">Aucune injection référencée</div>
  </div>
</template>
...
export default {
  ...
  watch: {
    "$store.state.value_history_injections": function () {
      if (
        Object.keys(this.$store.getters.value_history_injections).length > 1
      ) {
        this.injections = this.$store.getters.value_history_injections;
        this.Show = true;
      }
    ...
    "$store.state.value_timestamp_injections": function () {
      if (
        Object.keys(this.$store.getters.value_timestamp_injections).length > 1
      )
        this.timestamp = this.$store.getters.value_timestamp_injections;
    },
  },
