...
const showHistoryByTimestamp = async (info, client, userToVerify) => {
    return new Promise((resolve, reject) => {
      var query =   { $and: [ { timestamp: info }, { userId:  new mongo.ObjectId(userToVerify._id) } ] }
        if (/[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])/.test(info))
        {
          query =  { $and: [ { timestamp :{'$regex' : info, '$options' : 'i'}},  { userId:  new mongo.ObjectId(userToVerify._id) } ] }
        }
